...
class PerformanceCollector:
    ...
    def get_metrics(
        self, name: Optional[str] = None, limit: int = 1000
...
class PerformanceMonitor:
    ...
    def _setup_prometheus_metrics(self) -> None:
        ...
        self.cache_hit_ratio = Gauge(
            "memorizer_cache_hit_ratio", "Cache hit ratio", registry=self.registry
        )
    ...
    def _check_alerts(self) -> None:
        ...
        try:
            ...
            for alert in triggered_alerts:
                logger.warning(
                    f"Alert triggered: {alert['rule_name']} - {alert['description']}"
                )
    ...
    def track_memory_operation(
        self, operation: str, tier: str, duration: float, success: bool = True
    ) -> None:
        ...
        self.memory_duration.labels(operation=operation, tier=tier).observe(duration)
    ...
    def track_database_operation(
        ...
        self.database_duration.labels(operation=operation, table=table).observe(
            duration
        )
    ...
    def _update_cache_hit_ratio(self) -> None:
        ...
        try:
            ...
            hit_count = self.cache_operations.labels(
                operation="get", status="hit"
            )._value._value
            miss_count = self.cache_operations.labels(
    ...
    def track_vector_operation(
        ...
        self.vector_duration.labels(operation=operation).observe(duration)

    def track_user_operation(
        self, user_id: str, operation: str, success: bool = True
    ) -> None:
            ...
            try:
                ...
                if labels:
                    for key, value in labels.items():
                        if key == "operation":
                            monitor.track_memory_operation(
                                value, labels.get("tier", "unknown"), duration, True
                            )
                        elif key == "db_operation":
                            monitor.track_database_operation(
                                value, labels.get("table", "unknown"), duration, True
                            )
                ...
                if labels:
                    for key, value in labels.items():
                        if key == "operation":
                            monitor.track_memory_operation(
                                value, labels.get("tier", "unknown"), duration, False
                            )
                        elif key == "db_operation":
                            monitor.track_database_operation(
                                value, labels.get("table", "unknown"), duration, False
                            )
                        elif key == "vector_operation":
