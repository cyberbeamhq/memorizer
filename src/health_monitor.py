...
class HealthCheckRegistry:
    ...
    def register(self, name: str, check_func: Callable, config: Dict[str, Any] = None) -> None:
...
class HealthMonitor:
    ...
    def _register_default_checks(self) -> None:
        ...
        self.registry.register("database", self._check_database, {
            "timeout": 5,
            "critical": True
        })
        
        # Cache health check
        self.registry.register("cache", self._check_cache, {
            "timeout": 3,
            "critical": False
        })
        
        # Vector database health check
        self.registry.register("vector_db", self._check_vector_db, {
            "timeout": 5,
            "critical": False
        })
        
        # Memory manager health check
        self.registry.register("memory_manager", self._check_memory_manager, {
            "timeout": 3,
            "critical": True
        })
        ...
        self.registry.register("external_services", self._check_external_services, {
            "timeout": 10,
            "critical": False
        })
        ...
        try:
            ...
            if openai_key:
                try:
                    ...
                    services_status["openai"] = "healthy" if response.status_code == 200 else "unhealthy"
                except Exception as e:
        ...
        with self.lock:
            ...
            for check in self.health_cache.values():
                ...
                if component_name not in components:
                    ...
                    elif check.status == HealthStatus.DEGRADED and components[component_name].status == HealthStatus.HEALTHY:
                        components[component_name].status = HealthStatus.DEGRADED
            
            # Create summary
            summary = {
                "total_checks": total_checks,
                "healthy_checks": sum(1 for c in self.health_cache.values() if c.status == HealthStatus.HEALTHY),
                "degraded_checks": sum(1 for c in self.health_cache.values() if c.status == HealthStatus.DEGRADED),
                "unhealthy_checks": sum(1 for c in self.health_cache.values() if c.status == HealthStatus.UNHEALTHY),
    ...
    def register_check(self, name: str, check_func: Callable, config: Dict[str, Any] = None) -> None:
