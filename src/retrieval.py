...
class RelevanceScorer:
    ...
    def tf_idf_score(self, query_terms: List[str], text_terms: List[str], corpus_size: int = 1000) -> float:
        """
...
class MemoryRanker:
    ...
    def calculate_recency_score(self, created_at: datetime, max_age_days: int = 365) -> float:
...
class HybridRetriever:
    ...
    def retrieve_from_database(
        ...
        try:
            ...
            if search_memories:
                logger.info(f"Found {len(search_memories)} memories via database search")
        ...
        # Filter and rank
        return self.ranker.filter_and_rank_memories(unique_memories, query, max_items=max_items)
    
    def retrieve_from_vector_db(self, user_id: str, query: str, max_items: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(f"Retrieved {len(normalized_results)} results from vector database")
...
def retrieve_context(
    ...
    high_quality_results = [r for r in db_results if r.get('relevance_score', 0) > fallback_threshold]
    
    if len(high_quality_results) >= min_db_results:
        logger.info(f"Found {len(high_quality_results)} high-quality DB results, no fallback needed")
    ...
    logger.info(f"Combined retrieval: {len(db_results)} DB + {len(vector_results)} vector = {len(combined_results)} total")
...
def test_retrieval_system():
    ...
    test_memories = [
        {
            'id': 1, 'content': 'User asked about refund policy for order', 
            'tier': 'very_new', 'created_at': datetime.now(),
            'metadata': {}
        },
        {
            'id': 2, 'content': 'Customer complained about shipping delays', 
            'tier': 'mid_term', 'created_at': datetime.now() - timedelta(days=5),
            'metadata': {}
        },
        {
            'id': 3, 'content': 'User praised fast delivery and good customer service', 
            'tier': 'long_term', 'created_at': datetime.now() - timedelta(days=30),
    ...
    for i, result in enumerate(results):
        print(f"{i+1}. Score: {result.get('relevance_score', 0):.3f} - {result['content'][:50]}...")
